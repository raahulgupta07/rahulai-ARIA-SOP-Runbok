...
- **Scaling (read-heavy / MS Teams):** sized for many concurrent readers, few writers.
  ...
  - **Q&A bank cache** — approved Q&A pairs are served with **zero LLM** on both the web chat **and the Microsoft Teams** path, so repeat SOP questions don't re-hit the model.
