...
```bash
docker compose -f docker-compose.prod.yml ps          # status  (or docker-compose.npm.yml for Mode 1)
docker compose -f docker-compose.prod.yml logs -f app  # live logs
...
This ships **production-first**. The installer sets `APP_ENV=production`, which
**refuses to start** unless a strong `JWT_SECRET` (≥32 chars) and `OPENROUTER_API_KEY`
are set — so you can't accidentally run insecure.

- **No open signup.** Self-registration is **OFF** by default — users come from
  **LDAP/SSO** or are **created by an admin** (Settings → Users). Turn it on in
  Settings → Authentication only if you want public signup.
- **Secrets** (`JWT_SECRET`, DB password) are auto-generated by `install.sh` into
  `.env.prod` (chmod 600, git-ignored). Never commit `.env.prod`.
- **Super-admin** is bootstrapped from `SUPERADMIN_EMAIL` / `SUPERADMIN_PASSWORD`
  on every boot. **To change the password:** edit `.env.prod` and
  `docker compose … up -d --force-recreate app`.
- **Roles** are enforced in the backend, not just the UI — plain admins get a 403
  on Settings/config endpoints; only super-admins can change auth, storage, roles.
- **HTTPS**: Mode 2 auto-renews the Let's Encrypt cert (certbot loop). Mode 1 —
  your proxy handles renewal.
- **Data** persists across upgrades/restarts: Postgres in the `docsensei_pg` volume,
  page images in `./data`. `update.sh` takes a `pg_dump` backup before every upgrade
  (saved in `backups/`); back that directory up off-box (or use managed RDS +
  `DATABASE_URL`).
- **Models**: all LLM calls go through **OpenRouter** (chat + ingest + vision).
  No model runs on your box — no GPU required.

> The root `docker-compose.yml` is for **local development** only (bind-mounted
> code, dev creds). For any real deployment use `install.sh` →
> `docker-compose.prod.yml` (built-in HTTPS) or `docker-compose.npm.yml` (your proxy).
...
- **Auth**: local (bcrypt) + **multiple** LDAP/AD directories (ldap3) + **multiple** SSO/OIDC providers (JWKS) — each enabled+complete provider shows as its own login button; merged by email, JWT; separate `/login/admin` super-admin URL
...
- **Settings** — a left rail (Overview · Users · Authentication · Fact Approval · Embed Widget · API Access · Microsoft 365 · Microsoft Teams · Knowledge (OKF) · Storage), every page **full-width** with one shared design language (status cards, row cards, pills, popup config modals, inline SVG icons). **Overview** = 2-col (KPI tiles + preferences + jump-to | account + system-status rail). **Authentication** = method status cards + **per-provider/per-directory cards**, each with an **Add / Configure popup** (SSO icon-picker + issuer/client/secret + redirect URI; LDAP host/bind/base/filter + Test) — works with Keycloak, Microsoft Entra/Azure AD, Google, Okta. **Fact Approval** = a fixed-column governance grid (Auto/Review + confidence + pending, all aligned). For admins.
