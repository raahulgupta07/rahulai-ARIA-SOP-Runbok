...
- **In-app `+ Add ▾` menu** (Workspace top bar, OpenWebUI-style dropdown): **Upload files** (one or many) · **Upload directory** (whole folder, recurses) · **Import from server** (admin — scans the mounted `./documents` folder and queues every new file, dedup by name) · **Import from S3** (admin, when configured) · **Import from SharePoint** (admin — pulls every PDF/image from a Microsoft 365 document library via Microsoft Graph; set tenant/client/site in the modal, app secret stays in `SHAREPOINT_CLIENT_SECRET`; optional `SHAREPOINT_SYNC_ENABLED` daemon keeps pulling new files on a schedule).
...
- **Scaling (read-heavy / MS Teams):** sized for many concurrent readers, few writers.
  ...
  - **Multiple uvicorn workers** — set `WEB_CONCURRENCY` (default 2). All background work (ingest worker, inbox watcher, daemons) is **leader-gated** via a Postgres advisory lock so exactly one worker runs it no matter how many you scale to. Raise db `max_connections` (compose `PG_MAX_CONNECTIONS`, default 200) so `WEB_CONCURRENCY × DB_POOL_MAX` fits with headroom.
  - **Q&A bank cache** — approved Q&A pairs are served with **zero LLM** on both the web chat **and the Microsoft Teams** path, so repeat SOP questions don't re-hit the model. Serving fires on first-turn questions (verified under an 80-concurrent load test: 80/80 served, ~10× faster than re-answering, with the DB pool and LLM cap holding steady).
  - **Backpressure** — `LLM_MAX_CONCURRENCY` (default 6) caps simultaneous user-facing LLM calls (excess queue) to avoid provider rate-limit storms; inbound Teams messages run on a bounded pool (`TEAMS_WORKERS`, default 8) instead of an unbounded thread-per-message.
